        ...
        for index in tqdm(range(len(self.text))):
            ...
            while True:
                ...
                if str(current_character) == 'nan':
                    break
                if current_character in new_pre_content:
...
test_dataset = TestClassificationDataset(train_ids,train_text,train_characters,train_label1,train_label2,train_label3,train_label4,train_label5,train_label6,maxlen=350,flag=False)
#到里面的classificationdataset才进行字符的切割以及划分
test_loader = DataLoader(test_dataset,batch_size=16)
...
import csv
with open(r'/home/xiaoguzai/程序/剧本角色情感识别/数据/主语上文全部内容nezha_base'+str(seed)+'_'+str(bestpoint)+"result.csv", 'w') as f:
